-- create database if not exists
CREATE DATABASE IF NOT EXISTS movie_booking_db;
USE movie_booking_db;

-- Users (equivalent to customers)
CREATE TABLE IF NOT EXISTS users (
  user_id INT AUTO_INCREMENT PRIMARY KEY,
  name VARCHAR(100),
  email VARCHAR(100) UNIQUE,
  password VARCHAR(255),
  is_admin TINYINT DEFAULT 0
);

-- Genres (equivalent to categories)
CREATE TABLE IF NOT EXISTS genres (
  genre_id INT AUTO_INCREMENT PRIMARY KEY,
  name VARCHAR(100) UNIQUE
);

-- Movies (equivalent to products)
CREATE TABLE IF NOT EXISTS movies (
  movie_id INT AUTO_INCREMENT PRIMARY KEY,
  genre_id INT,
  title VARCHAR(150),
  price DECIMAL(10,2),
  available_seats INT,
  description TEXT,
  duration INT,
  showtime DATETIME,
  FOREIGN KEY (genre_id) REFERENCES genres(genre_id) ON DELETE SET NULL
);

-- Watchlist (equivalent to cart)
CREATE TABLE IF NOT EXISTS watchlist (
  watchlist_id INT AUTO_INCREMENT PRIMARY KEY,
  user_id INT,
  movie_id INT,
  seats_selected INT,
  added_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
  FOREIGN KEY (movie_id) REFERENCES movies(movie_id) ON DELETE CASCADE
);

-- Bookings (equivalent to orders)
CREATE TABLE IF NOT EXISTS bookings (
  booking_id INT AUTO_INCREMENT PRIMARY KEY,
  user_id INT,
  booking_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  status VARCHAR(30) DEFAULT 'confirmed',
  FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE
);

-- Booking Items
CREATE TABLE IF NOT EXISTS booking_items (
  booking_item_id INT AUTO_INCREMENT PRIMARY KEY,
  booking_id INT,
  movie_id INT,
  seats_booked INT,
  price DECIMAL(10,2),
  FOREIGN KEY (booking_id) REFERENCES bookings(booking_id) ON DELETE CASCADE,
  FOREIGN KEY (movie_id) REFERENCES movies(movie_id) ON DELETE SET NULL
);

-- Payments
CREATE TABLE IF NOT EXISTS payments (
  payment_id INT AUTO_INCREMENT PRIMARY KEY,
  booking_id INT,
  payment_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  amount DECIMAL(10,2),
  method VARCHAR(30),
  status VARCHAR(30),
  FOREIGN KEY (booking_id) REFERENCES bookings(booking_id) ON DELETE CASCADE
);

-- Reviews
CREATE TABLE IF NOT EXISTS reviews (
  review_id INT AUTO_INCREMENT PRIMARY KEY,
  user_id INT,
  movie_id INT,
  rating INT,
  comment TEXT,
  review_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE SET NULL,
  FOREIGN KEY (movie_id) REFERENCES movies(movie_id) ON DELETE CASCADE
);

-- Insert a default admin
INSERT IGNORE INTO users (name, email, password, is_admin)
VALUES ('Admin', 'admin@moviebooking.com', 'admin123', 1);

-- Insert genres
INSERT INTO genres (genre_id, name) VALUES
(1, 'Action'),
(2, 'Comedy'),
(3, 'Drama'),
(4, 'Horror'),
(5, 'Sci-Fi');

-- Insert movies
INSERT INTO movies (movie_id, genre_id, title, price, available_seats, description, duration, showtime) VALUES
(1, 1, 'Fast & Furious X', 299.00, 50, 'High-octane action thriller', 150, '2025-10-05 18:00:00'),
(2, 1, 'Mission Impossible 8', 349.00, 45, 'Tom Cruise returns with impossible stunts', 160, '2025-10-05 21:00:00'),
(3, 2, 'Laugh Out Loud', 199.00, 60, 'Comedy of the year', 120, '2025-10-06 15:00:00'),
(4, 2, 'The Funny Bone', 199.00, 55, 'Stand-up comedy special', 110, '2025-10-06 19:00:00'),
(5, 3, 'The Last Dance', 249.00, 40, 'Emotional drama about dreams', 140, '2025-10-07 17:00:00'),
(6, 3, 'Broken Wings', 249.00, 35, 'A story of resilience', 135, '2025-10-07 20:00:00');